...
v_dir = base_dir / "storage" / "assets" / "real_estate_verified"
r_dir = base_dir / "storage" / "assets" / "real_estate_raw"
...
# Load REAL authentic photos
img_construction = Image.open(v_dir / "real_concrete_building_india.jpg").convert("RGB")
img_society_towers = Image.open(v_dir / "real_indian_highrise_towers.jpg").convert("RGB")
img_cybercity = Image.open(v_dir / "real_cyber_city_gurgaon.jpg").convert("RGB")
img_keys = Image.open(r_dir / "real_house_keys.jpg").convert("RGB")
img_interior = Image.open(v_dir / "real_apartment_interior_verified.jpg").convert("RGB")
img_advisory = Image.open(v_dir / "real_business_meeting_office.jpg").convert("RGB")
img_noida_skyline = Image.open(v_dir / "real_noida_skyline_verified.jpg").convert("RGB")
...
right_half = fit_cover(img_society_towers, 538, 960, scale_boost=1.05)
...
# Yellow and White comparison badges
...
# Subtitles ONLY IN YELLOW (255, 215, 0) & WHITE (255, 255, 255)
subtitles = [
    ...
    (3.80, 5.60, "LOCATION & CONNECTIVITY", "GURGAON • METRO • PRIME ACCESS", (255, 215, 0), (255, 255, 255)),
...
for frame_idx in range(total_frames):
    ...
    # 2. Beat 2 [2.40s - 3.80s]: SPLIT_50_50 (Real Indian Price Trap vs Completed Society)
    elif t < 3.80:
        p = (t - 2.40) / 1.40
        scale = 1.0 + 0.04 * p
    ...
    # 3. Beat 3 [3.80s - 9.20s]: FULL_EXPLAINER (Real 3 Pillars of Indian Real Estate)
        ...
        if t < 5.60:
            # Pillar 1: Location & DLF Cyber City Gurgaon / Expressway
            p = (t - 3.80) / 1.80
            scale = 1.02 + 0.06 * p
            canvas = fit_cover(img_cybercity, 1080, 1920, scale_boost=scale, pan_x=p * 0.15)
        elif t < 7.40:
            # Pillar 2: Project Quality & Highrise Gated Society
            p = (t - 5.60) / 1.80
            scale = 1.02 + 0.06 * p
            canvas = fit_cover(img_society_towers, 1080, 1920, scale_boost=scale, pan_y=p * 0.15)
        else:
            # Pillar 3: Timely Possession & Real Key Handover
            p = (t - 7.40) / 1.80
            scale = 1.02 + 0.06 * p
            canvas = fit_cover(img_keys, 1080, 1920, scale_boost=scale, pan_y=-p * 0.1)
    ...
    # 5. Beat 5 [12.20s - 15.00s]: SPLIT_50_50 (Real Finished Apartment Living Room)
    elif t < 15.00:
        p = (t - 12.20) / 2.80
        scale = 1.02 + 0.05 * p
        pan = -0.15 + 0.3 * p
    ...
    # 6. Beat 6 [15.00s - 21.00s]: SPLIT_50_50 (Real Advisory Meeting + Official Logo)
    elif t < 21.00:
        p = (t - 15.00) / 6.00
        scale = 1.02 + 0.06 * p
        pan = -0.15 + 0.3 * p
    ...
    # 7. Beat 7 [21.00s - 28.50s]: FULL_EXPLAINER (Real Delhi NCR Skyline)
        ...
        p = (t - 21.00) / 7.50
        scale = 1.02 + 0.07 * p
        canvas = fit_cover(img_noida_skyline, 1080, 1920, scale_boost=scale, pan_y=p * 0.12).convert("RGBA")
...
audit_vid.release()
print("VERIFIED REAL RAW PHOTO MASTER RENDER COMPLETED SUCCESSFULLY!")
