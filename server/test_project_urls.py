...
urls_to_test = [
    ...
    f"https://aisandbox-pa.googleapis.com/v1/projects/{PROJECT_ID}/video:batchAsyncGenerateVideoStartImage",
    f"https://aisandbox-pa.googleapis.com/v1/projects/{PROJECT_ID}/operations",
...
for u in urls_to_test:
    try:
        r = requests.post(u, headers=headers, json=payload, timeout=8)
        print(f"[POST] {u.split('?')[0]} -> {r.status_code}")
        print(f"   {r.text[:300]}")
